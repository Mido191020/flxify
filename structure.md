...
├── vscode-extension/          # VS Code Extension (self-contained)
│   ├── package.json           # Extension manifest (name, commands, keybindings)
│   ├── tsconfig.json          # TypeScript config (excludes scripts from TS compilation)
│   ├── icon.png               # 128x128 marketplace icon
│   ├── README.md              # Marketplace description page with examples
│   ├── CHANGELOG.md           # Version history
│   ├── LICENSE                # MIT license
│   ├── CLAUDE.md              # VS Code extension project knowledge
│   ├── PLAN.MD                # Original extension requirements
│   ├── .vscodeignore          # Files excluded from .vsix package
│   ├── .vscode/launch.json    # F5 debug configuration (Extension Development Host)
│   ├── src/
│   │   ├── extension.ts       # Core: BoopState bridge, require shim, script loader, QuickPick command
│   │   └── scripts/           # 107 scripts + lib/ (copied from web app)
│   │       ├── *.js           # 107 script files
│   │       └── lib/           # 7 lib modules
│   └── out/
...
│       ├── plan-developer.md  # Full-stack + SEO + VS Code extension developer agent
...
### Architecture
```
User selects text → Cmd+Shift+B → QuickPick menu (107 scripts)
                                        ↓
                         BoopState bridge (VS Code ↔ script API)
                                        ↓
                         Script runs: state.text = transform(state.text)
                                        ↓
                         editor.edit() replaces text (single undo step)
```

### Key Differences from Web App
| Aspect | Web App | VS Code Extension |
|--------|---------|-------------------|
| Runtime | Browser (CSP-restricted) | Node.js (unrestricted) |
| Script loading | Build-time inlining | Runtime `new Function()` |
| Notifications | DOM toast system | `vscode.window` API |
| Editor | CodeMirror 6 | VS Code editor API |
| Module loading | Build-time IIFE wrapping | Runtime CommonJS sandbox |

### Extension Workflow
- `npm run compile` → TypeScript → `out/extension.js`
- F5 in VS Code → Extension Development Host
- `npx @vscode/vsce package` → `.vsix` file
